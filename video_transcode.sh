...
TIMESTAMP=$5
	...
	if [ "$SKIP_TRANSCODE" = true ] && [ "$RIPMETHOD" = "mkv" ]; then
		...
		$HANDBRAKE_CLI -i "$SRC" -o "$DEST/$LABEL.$DEST_EXT" --main-feature $HB_PRESET_PARAMETER"$HB_PRESET" $HB_ARGS 2>> "$LOG"
		rmdir "$SRC"	
		...
		do
			...
			if [ $SEC -gt "$MINLENGTH" ]; then
				...
				# shellcheck disable=SC2086
				$HANDBRAKE_CLI -i "$SRC" -o "$DEST/$LABEL-$TITLE.$DEST_EXT" --min-duration="$MINLENGTH" -t "$TITLE" $HB_PRESET_PARAMETER"$HB_PRESET" $HB_ARGS 2  >> "$LOG"
		...
		# shellcheck disable=SC2086
        $HANDBRAKE_CLI -i "$DEVNAME" -o "${DEST}/${LABEL}.${DEST_EXT}" --main-feature $HB_PRESET_PARAMETER"$HB_PRESET" $HB_ARGS 2>> "$LOG"
		eject "$DEVNAME"
	else
		...
		# shellcheck disable=SC2045
			...
			echo "Transcoding file $FILE" >> "$LOG"
				# shellcheck disable=SC2086
                $HANDBRAKE_CLI -i "$SRC/$FILE" -o "$DEST/$filename.$DEST_EXT" $HB_PRESET_PARAMETER"$HB_PRESET" $HB_ARGS 2>> "$LOG"
			rm "$SRC/$FILE"
       		done
		rmdir "$SRC"
	fi
